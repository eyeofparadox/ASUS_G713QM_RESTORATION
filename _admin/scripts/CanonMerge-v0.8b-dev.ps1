param([string]$SourceRoot, [string]$TargetRoot, [switch]$SimulateOnly, [string]$LogTail, [switch]$EnableHashFallback)
...
foreach ($rel in $sourceMap.Keys) {
    ...
    if (-not $src) {
        Write-Host "[ERR] No source file resolved for key: $rel"
        continue
    }
